...
public class AddRestaurantJPanel extends javax.swing.JPanel {
    ...
    public AddRestaurantJPanel(JPanel cardSequencePanel, MongoDatabase databases, Business business) {
        initComponents();
    ...
    String RestaurantName = txtFactoryName.getText().trim();
    String RestaurantAddress = txtFactoryAddress.getText().trim();
    String RestaurantContact = txtFactoryContact.getText().trim();
    String RestaurantEmail = txtFactoryEmail.getText().trim();
    String managerName = txtFactoryManagerName.getText().trim();
    String managerPassword = txtFactoryManagerPassword.getText().trim();

    // Validate inputs
    if (RestaurantName.isEmpty() ||RestaurantAddress.isEmpty() || RestaurantContact.isEmpty()
            || RestaurantEmail.isEmpty() || managerName.isEmpty() || managerPassword.isEmpty()) {
        JOptionPane.showMessageDialog(this, "All fields are mandatory.", "Error", JOptionPane.ERROR_MESSAGE);
        return;
    }

    if (!RestaurantEmail.matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
    ...
    try {
        ...
        Document existingFactory = supplierCollection.find(new Document("RestaurantEmail", RestaurantEmail)).first();
        if (existingFactory != null) {
            JOptionPane.showMessageDialog(this, "A Restaurant with this email already exists.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        Document foodFactory = new Document("RestaurantName", RestaurantName)
                .append("RestaurantAddress", RestaurantAddress)
                .append("RestaurantContact", RestaurantContact)
                .append("RestaurantEmail", RestaurantEmail)
                .append("RestaurantAdminId",  String.valueOf(userDoc.getObjectId("_id")))
                .append("organizationType", "Restaurant");
